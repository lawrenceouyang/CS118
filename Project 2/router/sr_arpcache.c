...
void sr_arpcache_sweepreqs(struct sr_instance *sr)
{
      ...
      requestIterator->next)
   {
      ...
      {
         /* Try, try again. */
         sr_link_send_arp_req(sr, requestIterator);
      ...
      else
      {
         ...
         for (packetIterator = requestIterator->packets; packetIterator != NULL; packetIterator =
            packetIterator->next)
         {
            sr_ip_send_type_three_icmp_pkt(sr, icmp_code_destination_host_unreachable,
...
void sr_arpcache_dump(struct sr_arpcache *cache)
{
   ...
   for (i = 0; i < SR_ARPCACHE_SZ; i++)
   {
      ...
      fprintf(stderr, "%.1x%.1x%.1x%.1x%.1x%.1x   %.8x   %.24s   %d\n", mac[0], mac[1], mac[2],
         mac[3], mac[4], mac[5], ntohl(cur->ip), ctime(&(cur->added)), cur->valid);
   }
