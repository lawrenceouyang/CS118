...
static inline uint16_t get_ip_header_len(sr_ip_hdr_t const * const pktPtr)
{
   return (pktPtr->ip_hl) * 4;
}

static inline sr_icmp_hdr_t * get_icmp_header(sr_ip_hdr_t * packetPtr)
{
   return (sr_icmp_hdr_t*) (((uint8_t*) packetPtr) + get_ip_header_len(packetPtr));

}

static inline sr_tcp_hdr_t * get_tcp_header(sr_ip_hdr_t * packetPtr)
{
   return (sr_tcp_hdr_t*) (((uint8_t*) packetPtr) + get_ip_header_len(packetPtr));
}
...
void ip_frwd_ip_pkt(struct sr_instance* sr, sr_ip_hdr_t* packet, unsigned int length,
   const struct sr_if* const rcvd_interface);
